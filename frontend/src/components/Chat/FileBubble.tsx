...
export const FileBubble: React.FC<FileBubbleProps> = ({ msg }) => {
	...
	return (
		<div className="mt-2 group/bubble max-w-[400px]">
			{hasPreview ? (
				<div className="relative rounded-lg overflow-hidden bg-[#2b2d31] border border-[#1e1f22] cursor-pointer hover:border-[#4752c4] transition group">
					{thumbUrl ? (
						<img
							src={thumbUrl}
							alt="thumbnail"
					...
					{isDecrypting && (
						<div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-4">
							...
							<div className="w-full bg-white/20 h-1 rounded-full overflow-hidden">
								<div
									className="bg-[#4752c4] h-full transition-all"
										...
										<div className="relative w-full h-full flex flex-col items-center justify-center">
											{fullUrl ? (
												...
												<div className="flex flex-col items-center gap-4">
													<Loader2
														size={48}
														className="text-[#4752c4] animate-spin"
													/>
													<div className="w-48 bg-white/20 h-1 rounded-full">
														<div
															className="bg-[#4752c4] h-full"
															style={{ width: `${progress}%` }}
														/>
													</div>
												</div>
											...
											<div className="absolute bottom-0 left-1/2 -translate-x-1/2 flex items-center gap-6 bg-black/60 px-6 py-3 rounded-full border border-white/10 backdrop-blur-md">
												...
												<button
													...
													className="text-white bg-[#5865f2] hover:bg-[#4752c4] px-4 py-1.5 rounded font-bold transition flex items-center gap-2"
												>
				...
				>
					<div className="w-10 h-10 bg-[#313338] rounded flex items-center justify-center text-[#949ba4] shrink-0">
						{isDecrypting ? (
							<Loader2 size={20} className="animate-spin text-[#4752c4]" />
						) : (
							<FileIcon size={20} />
						)}
					</div>
					...
					{isDecrypting && (
						<div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#4752c4]/20 rounded-b-md">
							<div
								className="h-full bg-[#4752c4] transition-all"
								style={{ width: `${progress}%` }}
							/>
						</div>
					)}
